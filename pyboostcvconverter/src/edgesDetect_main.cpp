...
Mat edgesDetectmain(_model model, Mat I, Mat chnsReg, Mat chnsSim, uint32*& ind, uint8*& segsOut, uint32*& segDims)
{
    ...
    string msgSharpen = "Model supports sharpening of at most %i pixels!\n";
    if (sharpen > nBnds - 1)
    {
        sharpen = nBnds - 1;
    ...
    // cout << "model.fids.cols : " << model.fids.cols << endl;
    const int nTreeNodes = model.fids.cols;
    // cout << "nTreeNodes : " << nTreeNodes << endl;
...
//    const int segDims[5] = {gtWidth, gtWidth, h1, w1, nTreesEval};
    segDims = new uint32[5];
    segDims[0] = gtWidth;
    segDims[1] = gtWidth;
    segDims[2] = h1;
    segDims[3] = w1;
    segDims[4] = nTreesEval;
    ...
    getadd<float>(chnsReg, chns);
    getadd<float>(chnsSim, chnsSs);

    // **********************test*************************
    // cout << "you construct lookup tables" << endl;
    //**********************test*************************

    // create outputs
    float *E = new float[h2 * w2];
    ind = new uint32[h1 * w1 * nTreesEval];

    segsOut = new uint8[gtWidth * gtWidth * h1 * w1 * nTreesEval];
    for (int i = 0; i < h2 * w2; i++)
        E[i] = 0.f;
    for (int i = 0; i < h1 * w1 * nTreesEval; i++)
        ind[i] = 0;
    for (int i = 0; i < gtWidth * gtWidth * h1 * w1 * nTreesEval; i++)
        segsOut[i] = 0;
    
    //**********************test*************************
    // cout << "create outputs" << endl;
    //**********************test*************************

    //****************problem**************

    // cout << "w1: " << w1 << endl;
    // cout << "nTreesEval: " << nTreesEval << endl;
    // cout << "h1: " << h1 << endl;
        ...
        for (int t = 0; t < nTreesEval; t++)
        {
            for (int r0 = 0; r0 < 2; r0++)
                for (int r = r0; r < h1; r += 2)
                {
                    ...
                    // cout << "idx: " << r + c * h1 + t * h1 * w1 << endl;
                }
        }
    }

    //**********************test*************************
    // cout << "apply forest to all patches and store leaf inds" << endl;
    //**********************test*************************
...
#pragma omp parallel for num_threads(nThreads)
    ...
    //**********************test*************************
    // cout << "compute edge maps (avoiding collisions from parallel executions)" << endl;
    //**********************test*************************
...
//                    uint8 S0[4096], *S = segsOut + (r + c * h1 + t * h1 * w1) * g * g;
                    uint8 *S = segsOut + (r + c * h1 + t * h1 * w1) * g * g;
    ...
    //**********************test*************************
    // cout << "computed sharpened edge maps, snapping to local color values" << endl;
    //**********************test*************************

    float_I.release();
    Mat Emat(h2, w2, CV_32FC1);
    fillmat<float>(E, Emat);
// //    Mat segmat(5, segDims, CV_8UC1, segsOut);
//     Mat indmat(3, indDims, CV_32SC1, ind);
// //    tuple<Mat, Mat, Mat> output = make_tuple(Emat, indmat, segmat);
//     tuple<Mat, Mat> output = make_tuple(Emat, indmat);
